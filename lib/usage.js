...
function normalizeDay(row) {
  ...
  if (!Number.isFinite(rawCost) || rawCost < 0 || rawCost > MAX_COST_USD) {
    throw new Error(`Invalid costUsd value in ccusage data for ${date}.`);
  }
...
function buildWirePayload(
  ...
  const safeTimezone = safeText(timezone, { maxLength: 64 });
  if (!safeTimezone || safeTimezone !== timezone || !TIMEZONE_PATTERN.test(safeTimezone)) {
