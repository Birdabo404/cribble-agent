...
function wireDay(row) {
  ...
  return {
    ...
    totalTokens,
    costUsd: row.costUsd,
    agentBreakdowns: row.agentBreakdowns
...
function modelsByUsage(row, date, total) {
  const rawModels = uniqueStrings(row.modelsUsed ?? row.models)
    .filter(isAttributedModel);
  let breakdowns = (Array.isArray(row.modelBreakdowns) ? row.modelBreakdowns : [])
    .map((breakdown) => {
      const model = safeText(breakdown?.modelName ?? breakdown?.name, {
        maxLength: 256,
      });
...
function normalizeDay(row) {
  ...
  return {
    ...
    agents: uniqueStrings([
      ...agentBreakdowns.map((breakdown) => breakdown.name),
      ...agents,
    ]),
    models,
    agentBreakdowns,
