...
const { dirname, posix, win32 } = require("node:path");
const properLockfile = require("proper-lockfile");
const { configDirectory: platformConfigDirectory } = require("./config-path");
...
function configDirectory(homeDirectory = homedir(), options = {}) {
  return platformConfigDirectory({ homeDirectory, ...options });
}

function syncStatePath(homeDirectory = homedir(), options = {}) {
  const platform = options.platform ?? process.platform;
  const pathApi = platform === "win32" ? win32 : posix;
  return pathApi.join(
    configDirectory(homeDirectory, { ...options, platform }),
    "sync-state.json",
  );
}

function syncLockPath(homeDirectory = homedir(), options = {}) {
  const platform = options.platform ?? process.platform;
  const pathApi = platform === "win32" ? win32 : posix;
  return pathApi.join(
    configDirectory(homeDirectory, { ...options, platform }),
...
function ensurePrivateDirectory(filePath) {
  ...
  try {
    ...
    // Windows enforces inherited ACLs rather than POSIX mode bits.
  }
}
...
async function withSyncLock(
  task,
  {
    ...
    acquireGuardFn = properLockfile.lock,
  } = {},
) {
  ensurePrivateDirectory(filePath);
  ...
  let releaseGuard;
  try {
    releaseGuard = await acquireGuardFn(filePath, {
      realpath: false,
      lockfilePath: `${filePath}.acquire`,
      stale: 30_000,
      update: 10_000,
      retries: {
        retries: 5,
    ...
    if (error?.code === "ELOCKED") throw new SyncAlreadyRunningError();
    throw error;
  }
  ...
  try {
    ...
    try {
      ...
      // Age alone never invalidates a lock whose owner is still alive. A slow
      // multi-distro collection can legitimately exceed ten minutes, and
      // evicting it would allow two uploads to overlap. The short-lived
      // proper-lockfile guard serializes stale cleanup and acquisition, so two
      // contenders cannot unlink each other's newly created lock.
      if (ownerRunning) throw new SyncAlreadyRunningError();
      unlinkSync(filePath);
      writeFileSync(
        filePath,
        `${JSON.stringify({ lockId, pid: process.pid, startedAt })}\n`,
