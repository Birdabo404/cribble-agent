...
function resolveBundledNativeBinary(
  baseDirectory,
  {
  ...
  if (!supported.has(platformArch)) return null;

  const packageName = `@ccusage/ccusage-${platformArch}`;
  const binaryName = platform === "win32" ? "ccusage.exe" : "ccusage";
  try {
    const packagePath = requireResolveFn(`${packageName}/package.json`, {
      paths: [baseDirectory],
    });
    const binaryPath = join(dirname(packagePath), "bin", binaryName);
    if (!existsSyncFn(binaryPath)) return null;
    if (platform === "win32") return binaryPath;
    try {
      accessSyncFn(binaryPath, constants.X_OK);
    } catch {
      // ccusage 20.0.20 is published without its POSIX executable bit. Repair
      // the pinned package in place, then verify that it can actually run.
      try {
        chmodSyncFn(binaryPath, 0o755);
        accessSyncFn(binaryPath, constants.X_OK);
      } catch {
        return null;
      }
...
function loadCcusage(
  ...
  const bundledNativeBinary = configuredBinary
    ? null
    : resolveBundledNativeBinary(baseDirectory, {
        accessSyncFn,
        arch,
        chmodSyncFn,
        existsSyncFn,
  ...
  const bundledBinary = configuredBinary || bundledNativeBinary
    ? null
  ...
  const command = configuredBinary || bundledNativeBinary || nodePath;
  ...
  const args = configuredBinary || bundledNativeBinary
    ? ["daily", "--by-agent", "--json", ...sinceArgs, ...timezoneArgs]
    : [bundledBinary, "daily", "--by-agent", "--json", ...sinceArgs, ...timezoneArgs];

  try {
    // Prefer the bundled native collector so a timeout terminates the actual
    // scanner. The JavaScript fallback uses this process's absolute Node path
    // because launchd starts with a minimal PATH.
