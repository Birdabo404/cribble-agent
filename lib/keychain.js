...
function throwStoreError(platform, action) {
  throw new Error(`Could not ${action} the Cribble Agent key in ${credentialStoreLabel(platform)}.`);
}
...
function keychainHasApiKey(options = {}) {
  ...
  if (result.status !== 0) throwStoreError(platform, "check");
  return true;
}
...
function readKeychainApiKey(options = {}) {
  ...
  if (result.status !== 0) throwStoreError(platform, "read");
...
async function promptAndStoreApiKey(options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error(`The Agent key was not saved to ${credentialStoreLabel(platform)}.`);
  }
}
...
function removeKeychainApiKey(options = {}) {
  ...
  if (result.status === NOT_FOUND_STATUS) return false;
  if (result.status !== 0) throwStoreError(platform, "remove");
  return true;
}
