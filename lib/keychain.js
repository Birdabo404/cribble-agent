...
function readLinuxApiKey(options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error("Could not read the Cribble Agent key from Linux Secret Service.");
  }
...
function storeLinuxApiKey(apiKey, options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error("The Agent key was not saved to Linux Secret Service.");
  }
}
...
function removeLinuxApiKey(options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error("Could not remove the Cribble Agent key from Linux Secret Service.");
  }
  return true;
}
...
function runPowerShell(script, input, options = {}) {
  ...
  if (result.status !== 0) {
    throw new Error("Windows could not protect the Cribble Agent key for this user.");
  }
...
function readHiddenLine({ input = process.stdin, output = process.stdout } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // isPaused() is false for a stream that was never started (flowing is
    // null, not false), so it cannot decide whether to pause afterwards. A
    // fresh process.stdin must be paused again after our resume() below, or
    // its referenced TTY handle keeps the CLI alive after the prompt ends.
