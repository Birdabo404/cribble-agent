...
const APPLICATION_MANAGED_DISTRIBUTIONS = Object.freeze([
  /^docker-desktop(?:-data)?$/i,
  /^rancher-desktop(?:-data)?$/i,
  /^podman-machine-/i,
]);
...
function discoverWslHomes({
  ...
  for (const distribution of distributions) {
    if (APPLICATION_MANAGED_DISTRIBUTIONS.some((pattern) => pattern.test(distribution))) {
      continue;
    }
    let linuxHome;
    try {
